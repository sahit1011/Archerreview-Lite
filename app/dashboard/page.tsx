...
function DashboardContent() {
  ...
  const [isRefreshingFocusAreas, setIsRefreshingFocusAreas] = useState<boolean>(false);
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Try to get userId from multiple sources in order of priority
        ...
        try {
          ...
          if (userStr) {
            ...
            // If we have a user in localStorage but no userId yet, use that
            if (!userId && userFromStorage && (userFromStorage.id || userFromStorage._id)) {
              userId = userFromStorage.id || userFromStorage._id;
            }
          }
        } catch (e) {
        ...
        console.log('Dashboard load - fromOnboarding:', fromOnboarding, 'userId:', userId,
                    ...
                    'userFromStorage:', userFromStorage ? 'present' : 'missing');

        if (!userId) {
          throw new Error('User not authenticated - no user ID found from any source');
        }

        // At this point we should have a userId from one of the sources
        ...
        try {
          ...
          // If we have a user from localStorage and API fetch failed, use that as fallback
          if (userFromStorage) {
            console.log('Using user from localStorage as fallback:', userFromStorage);
            setUser(userFromStorage);
          } else {
            throw new Error('Failed to fetch user data and no fallback available');
          }
        }
        ...
        if (user && user._id) {
          ...
          console.error('User not found or missing ID - all attempts to get user data failed');

          // Check localStorage one more time as a last resort
          const lastResortUserId = localStorage.getItem('userId');
          const lastResortUserStr = localStorage.getItem('user');

          console.log('Last resort check - userId in localStorage:', lastResortUserId);
          console.log('Last resort check - user in localStorage:', lastResortUserStr ? 'present' : 'missing');

          if (lastResortUserId && lastResortUserStr) {
            try {
              const lastResortUser = JSON.parse(lastResortUserStr);
              console.log('Using last resort user from localStorage:', lastResortUser);
              setUser(lastResortUser);
            } catch (e) {
              console.error('Error parsing last resort user from localStorage:', e);
              throw new Error('User not found or missing ID - all attempts failed');
            }
          } else {
            throw new Error('User not found or missing ID - all attempts failed');
          }
        ...
        if (err instanceof Error && err.message.includes('User not found or missing ID')) {
          setError('Unable to find your user account. Please try logging in again or complete the onboarding process.');
        } else {
  ...
  const handleRefreshFocusAreas = async () => {
    ...
    try {
      setIsRefreshingFocusAreas(true);
      console.log('Refreshing focus areas...');
      ...
      const data = await response.json();
      console.log('Readiness score API response:', data);

      if (data.success) {
        setReadinessScore(data.readinessScore);
        console.log('Updated readiness score:', data.readinessScore);

        // Update focus areas
        const weakAreas = data.readinessScore?.weakAreas || [];
        const strongAreas = data.readinessScore?.strongAreas || [];
        ...
        const areas = [
          ...
          ...strongAreas.slice(0, 1).map((area: any) => ({
            ...area,
            type: 'strong',
            message: 'You\'re doing well here! Continue practicing to maintain your strong performance.'
          }))
        ...
        console.log('Setting focus areas:', areas);

        // Force a state update with a new array reference
        setFocusAreas([...areas]);

        // If there are no focus areas after refresh, show a message
        if (areas.length === 0) {
          console.warn('No focus areas returned from API after refresh');
        }
      } else {
        console.error('Failed to refresh focus areas:', data.message || 'Unknown error');
      }
    } catch (err) {
      console.error('Error refreshing focus areas:', err);
    } finally {
      setIsRefreshingFocusAreas(false);
    }
  };
  ...
  const handleGenerateRemediationSuggestions = async () => {
    ...
    try {
      ...
      console.log('Remediation suggestions generated successfully');
    } catch (error) {
    ...
    <ProtectedRoute>
      ...
      {user && (
        <CombinedFocusRemediationCard
          ...
          userId={user._id}
          isRefreshingFocusAreas={isRefreshingFocusAreas}
