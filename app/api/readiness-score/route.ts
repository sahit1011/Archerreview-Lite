...
export async function POST(request: NextRequest) {
  try {
    ...
    let readinessScore = await calculateReadinessScore(body.userId);
    ...
    // Populate the weak and strong areas
    readinessScore = await ReadinessScore.findById(readinessScore._id)
      .populate('weakAreas')
      .populate('strongAreas');
    ...
    if (!readinessScore) {
      return NextResponse.json(
        {
          success: false,
          message: 'Failed to populate readiness score data.'
        },
        { status: 500 }
      );
    }
