...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = {
      ...
      topicPerformance: readinessScore ? readinessScore.categoryScores.map((c: any) => ({
        topicName: c.category.split('_').map((word: string) => word.charAt(0) + word.slice(1).toLowerCase()).join(' '),
        score: c.score
      })) : [],
      studyPatterns: [
