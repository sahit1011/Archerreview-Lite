...
const PlanVersionManagement: React.FC<PlanVersionManagementProps> = ({ userId }) => {
  ...
  const getChangeTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'REVIEW_ADDED':
        return (
          <svg className="h-5 w-5 text-accent-foreground" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
  ...
  const getCreatorBadge = (creator: string) => {
    switch (creator) {
      case 'USER':
        return <span className="bg-primary/10 text-primary text-xs px-2 py-1 rounded-full">User</span>;
      case 'ADAPTATION_AGENT':
        return <span className="bg-primary/10 text-primary text-xs px-2 py-1 rounded-full">Adaptation Agent</span>;
      case 'EVOLUTION_AGENT':
        return <span className="bg-accent text-accent-foreground text-xs px-2 py-1 rounded-full">Evolution Agent</span>;
      default:
        return <span className="bg-muted text-muted-foreground text-xs px-2 py-1 rounded-full">System</span>;
    }
  };
    ...
    return (
      <div className="bg-card rounded-xl shadow-card hover:shadow-card-hover transition-all p-6 mb-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-xl font-semibold text-foreground">Plan Version Management</h2>
        </div>
        <div className="flex flex-col items-center justify-center h-64">
          <div className="w-16 h-16 border-t-4 border-primary border-solid rounded-full animate-spin"></div>
          <p className="mt-4 text-muted-foreground">Loading plan versions...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-card rounded-xl shadow-card hover:shadow-card-hover transition-all p-6 mb-6">
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex space-x-2">
          <button
            ...
            className={`px-3 py-1 rounded-lg text-sm shadow-button ${showCompare ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}
          >
      ...
      <div className={`grid ${showCompare ? 'grid-cols-1 md:grid-cols-2 gap-6' : 'grid-cols-1'}`}>
        ...
        <div className="bg-secondary p-5 rounded-lg shadow-card">
          <h3 className="text-lg font-medium text-foreground mb-4">Plan Versions</h3>
            ...
            {planVersions.map((version) => (
              ...
              >
                <div className="flex justify-between items-center">
                  <div className="flex items-center">
                    <DocumentDuplicateIcon className="h-5 w-5 text-primary mr-2" />
                    <span className="font-medium text-foreground">Version {version.versionNumber}</span>
                  </div>
                  {version.isActive && (
                    <span className="bg-success/10 text-success text-xs px-2 py-1 rounded-full">Active</span>
                  )}
                </div>
                <div className="mt-2 text-sm text-muted-foreground">{version.description}</div>
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              {!selectedVersionData.isActive && !showCompare && (
                <button
                  ...
                  className="flex items-center bg-primary text-primary-foreground px-3 py-1 rounded-lg text-sm shadow-button hover:bg-primary/90 transition-colors"
                >
            ...
            {showCompare && compareVersionData ? (
              <>
                ...
                <div className="mb-4 grid grid-cols-2 gap-4">
                  <div key="selected-version" className="p-4 bg-muted rounded-lg">
                    <div key="selected-version-label" className="text-sm text-muted-foreground mb-1">Version {selectedVersionData.versionNumber}</div>
                    <div key="selected-version-desc" className="text-foreground font-medium">{selectedVersionData.description}</div>
                    <div key="selected-version-date" className="mt-2 text-xs text-muted-foreground">
                  ...
                  <div key="compare-version" className="p-4 bg-muted rounded-lg">
                    <div key="compare-version-label" className="text-sm text-muted-foreground mb-1">Version {compareVersionData.versionNumber}</div>
                    <div key="compare-version-desc" className="text-foreground font-medium">{compareVersionData.description}</div>
                    <div key="compare-version-date" className="mt-2 text-xs text-muted-foreground">
                ...
                {/* Metrics Comparison */}
                <div className="mb-4">
                  <h4 className="text-md font-medium text-foreground mb-2">Metrics Comparison</h4>
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={metric.id} className="bg-muted p-3 rounded-lg">
                        <div key={`metric-label-${metric.id}`} className="text-sm font-medium text-foreground mb-2">{metric.label}</div>
                        <div key={`metric-grid-${metric.id}`} className="grid grid-cols-2 gap-4">
                          <div key={`${metric.id}-v1`}>
                            <div key={`${metric.id}-v1-label`} className="text-xs text-muted-foreground mb-1">Version {selectedVersionData.versionNumber}</div>
                            <div key={`${metric.id}-v1-value`} className="text-lg font-medium text-foreground">{metric.value1}</div>
                          </div>
                          <div key={`${metric.id}-v2`}>
                            <div key={`${metric.id}-v2-label`} className="text-xs text-muted-foreground mb-1">Version {compareVersionData.versionNumber}</div>
                            <div key={`${metric.id}-v2-value`} className="text-lg font-medium text-foreground">{metric.value2}</div>
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                {/* Changes Comparison */}
                <div className="mb-4">
                  <h4 className="text-md font-medium text-foreground mb-2">Changes</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div key="selected-version-changes" className="bg-muted p-3 rounded-lg">
                      <div className="text-xs text-muted-foreground mb-2">Version {selectedVersionData.versionNumber} Changes</div>
                      {selectedVersionData.changes.length > 0 ? (
                        <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
                          {selectedVersionData.changes.map((change, index) => (
                            <div
                              key={`selected-${change.type}-${index}`}
                              className="bg-muted p-2 rounded-lg flex items-start"
                            >
                              <div key={`selected-icon-${index}`} className="flex-shrink-0 mt-1">
                                {getChangeTypeIcon(change.type)}
                              </div>
                              <div key={`selected-content-${index}`} className="ml-2">
                                <div key={`selected-title-${index}`} className="text-xs font-medium text-foreground">{change.type.split('_').join(' ')}</div>
                                <div key={`selected-desc-${index}`} className="text-xs text-muted-foreground">{change.description}</div>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    <div key="compare-version-changes" className="bg-muted p-3 rounded-lg">
                      <div className="text-xs text-muted-foreground mb-2">Version {compareVersionData.versionNumber} Changes</div>
                          ...
                          {compareVersionData.changes.map((change, index) => (
                            ...
                            >
                              ...
                              <div key={`compare-content-${index}`} className="ml-2">
                                <div key={`compare-title-${index}`} className="text-xs font-medium text-foreground">{change.type.split('_').join(' ')}</div>
                                <div key={`compare-desc-${index}`} className="text-xs text-muted-foreground">{change.description}</div>
                              </div>
                            </div>
                          ))}
                        </div>
              ...
              <>
                ...
                <div className="mb-4 p-4 bg-muted rounded-lg">
                  <div className="text-sm text-muted-foreground mb-1">Reason for Change</div>
                  <div className="text-foreground">{selectedVersionData.reason}</div>
                  <div className="mt-3 text-sm text-muted-foreground mb-1">Description</div>
                  <div className="text-foreground">{selectedVersionData.description}</div>
                  <div className="mt-3 text-sm text-muted-foreground mb-1">Created</div>
                ...
                <div className="mb-4">
                  <h4 className="text-md font-medium text-foreground mb-2">Plan Metrics</h4>
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      <div key={metric.id} className="bg-muted p-3 rounded-lg">
                        <div key={`regular-metric-label-${metric.id}`} className="text-xs text-muted-foreground mb-1">{metric.label}</div>
                        <div key={`regular-metric-value-${metric.id}`} className="text-lg font-medium text-foreground">{metric.value}</div>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
                      {selectedVersionData.changes.map((change, index) => (
                        <div
                          ...
                          className="bg-muted p-3 rounded-lg flex items-start"
                        >
                          <div key={`regular-icon-${index}`} className="flex-shrink-0 mt-1">
                            {getChangeTypeIcon(change.type)}
                          </div>
                          <div key={`regular-content-${index}`} className="ml-3">
                            <div key={`regular-title-${index}`} className="text-sm font-medium text-foreground">{change.type.split('_').join(' ')}</div>
                            <div key={`regular-desc-${index}`} className="text-sm text-muted-foreground">{change.description}</div>
                          </div>
                        </div>
                      ))}
                    </div>
