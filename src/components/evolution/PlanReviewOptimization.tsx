...
const PlanReviewOptimization: React.FC<PlanReviewOptimizationProps> = ({ userId }) => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'COMPLETED':
        return <span className="bg-success/10 text-success text-xs px-2 py-1 rounded-full">Completed</span>;
      case 'IN_PROGRESS':
        return <span className="bg-primary/10 text-primary text-xs px-2 py-1 rounded-full">In Progress</span>;
      case 'PENDING':
        return <span className="bg-warning/10 text-warning text-xs px-2 py-1 rounded-full">Pending</span>;
      case 'SCHEDULED':
        return <span className="bg-accent text-accent-foreground text-xs px-2 py-1 rounded-full">Scheduled</span>;
      default:
        return <span className="bg-muted text-muted-foreground text-xs px-2 py-1 rounded-full">{status}</span>;
    }
  };
  ...
  const getImpactBadge = (impact: string) => {
    switch (impact) {
      case 'HIGH':
        return <span className="bg-success/10 text-success text-xs px-2 py-1 rounded-full">High Impact</span>;
      case 'MEDIUM':
        return <span className="bg-primary/10 text-primary text-xs px-2 py-1 rounded-full">Medium Impact</span>;
      case 'LOW':
        return <span className="bg-muted text-muted-foreground text-xs px-2 py-1 rounded-full">Low Impact</span>;
      default:
        return null;
    }
  };
    ...
    return (
      <div className="bg-card rounded-xl shadow-card hover:shadow-card-hover transition-all p-6 mb-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-xl font-semibold text-foreground">Plan Review & Optimization</h2>
        </div>
        <div className="flex flex-col items-center justify-center h-64">
          <div className="w-16 h-16 border-t-4 border-primary border-solid rounded-full animate-spin"></div>
          <p className="mt-4 text-muted-foreground">Loading plan reviews...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-card rounded-xl shadow-card hover:shadow-card-hover transition-all p-6 mb-6">
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex space-x-2">
          <button
            ...
            className="px-3 py-1 rounded-lg text-sm shadow-button bg-muted text-muted-foreground hover:bg-muted/70 transition-colors"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Review List */}
        <div className="md:col-span-1 bg-secondary p-5 rounded-lg shadow-card">
          <h3 className="text-lg font-medium text-foreground mb-4">Review History</h3>
        ...
        {/* Review Details */}
        {selectedReviewData && (
          <div className="md:col-span-2 bg-secondary p-5 rounded-lg shadow-card">
            ...
            <div className="mb-4 p-4 bg-muted rounded-lg">
              <div className="text-foreground">{selectedReviewData.summary}</div>
            </div>

            <div className="mb-4">
              <h4 className="text-md font-medium text-foreground mb-2">Expected Improvements</h4>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-muted p-3 rounded-lg">
                  <div className="text-xs text-muted-foreground mb-1">Performance</div>
                  <div className="text-lg font-medium text-success">+{selectedReviewData.metrics.performanceImprovement}%</div>
                </div>
                <div className="bg-muted p-3 rounded-lg">
                  <div className="text-xs text-muted-foreground mb-1">Time Saved</div>
                  <div className="text-lg font-medium text-primary">{selectedReviewData.metrics.timeOptimization}%</div>
                </div>
                <div className="bg-muted p-3 rounded-lg">
                  <div className="text-xs text-muted-foreground mb-1">Topic Coverage</div>
                  <div className="text-lg font-medium text-accent-foreground">+{selectedReviewData.metrics.topicCoverageIncrease}%</div>
                </div>
                <div className="bg-muted p-3 rounded-lg">
                  <div className="text-xs text-muted-foreground mb-1">Readiness Boost</div>
                  <div className="text-lg font-medium text-warning">+{selectedReviewData.metrics.readinessBoost}%</div>
                </div>
              </div>
            </div>

            <div>
              <h4 className="text-md font-medium text-foreground mb-2">Optimization Suggestions</h4>
              <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                            ...
                            <button
                              ...
                              className={`px-3 py-1 rounded text-xs flex items-center ${
                                suggestion.approved
                                  ...
                                  : 'bg-secondary text-foreground hover:bg-success/10 hover:text-success'
                              }`}
                            >
